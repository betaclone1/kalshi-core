...
DB_TRADES_PATH = os.path.join(os.path.dirname(__file__), "trades.db")
...
def fetch_all_trades():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, date, time, strike, side, price, position, status, contract FROM trades ORDER BY id DESC")
    rows = cursor.fetchall()
    conn.close()
    return [dict(zip(["id","date","time","strike","side","price","position","status","contract"], row)) for row in rows]
...
def insert_trade(trade):
    ...
    cursor.execute(
        ...
        (trade['date'], trade['time'], trade['strike'], trade['side'], trade['price'], trade['position'], trade.get('status', 'open'), trade.get('contract'))
    )
    conn.commit()
